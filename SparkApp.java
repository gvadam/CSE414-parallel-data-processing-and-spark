...
public class SparkApp {
 ...
 |-- marketid: integer (nullable = true)
 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> seattle = rdd.filter(r -> r.get(ORIGIN_CITY_NAME).toString().equals("Seattle, WA"));
        JavaRDD<Row> destCities = seattle.map(r -> RowFactory.create(r.get(DEST_CITY_NAME)));
        return destCities.distinct();
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> cities = rdd.filter(r -> r.get(CANCELLED).toString().equals("0"));
        JavaPairRDD<Tuple2<String, Integer>, Integer> ones = cities.mapToPair(r ->
                new Tuple2<Tuple2<String, Integer>, Integer>(new Tuple2<String, Integer>(r.get(ORIGIN_CITY_NAME).toString(), r.getInt(MONTH)), 1));
        JavaPairRDD<Tuple2<String, Integer>, Integer> counts = ones.reduceByKey((v1, v2) -> v1 + v2);
        JavaRDD<Row> output = counts.map(r -> RowFactory.create(r._1(), r._2()));
        return output;
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        JavaRDD<Row> cities = rdd.filter(r -> r.get(CANCELLED).toString().equals("0"));
        JavaRDD<Row> noNulls = cities.filter(r -> !r.get(DEP_DELAY).equals(null));

        JavaPairRDD<String, Tuple2<Integer, Integer>> delays = noNulls.mapToPair(r ->
                new Tuple2<String, Tuple2<Integer, Integer>>(r.getString(ORIGIN_CITY_NAME),
                        new Tuple2<Integer, Integer>(r.getInt(DEP_DELAY), 1)));
        JavaPairRDD<String, Tuple2<Integer, Integer>> sums = delays.reduceByKey((v1, v2) ->
                new Tuple2<Integer, Integer>(v1._1() + v2._1(), v1._2() + v2._2()));
        JavaPairRDD<String, Double> avg = sums.mapToPair(t ->
                new Tuple2<String, Double>(t._1(), 1.0*(t._2()._1()) / t._2()._2()));
        JavaRDD<Row> output = avg.map(r -> RowFactory.create(r._1(), r._2()));
        return output;
    }
